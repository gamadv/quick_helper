...
export function reverseOnlyLetters(S: string) {
  const isLetter = (letterSelected: string) => letterSelected.toLowerCase() !== letterSelected.toUpperCase();
  let letters = [...S].filter(c => isLetter(c));
  let reversed = [...S].map((c) => isLetter(c) ? letters.pop() : c).join('');
  return reversed
}
